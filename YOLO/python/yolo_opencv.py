...
ap.add_argument('-c', '--config', 
                help = 'path to config file', default="../yolov3-tiny.cfg")
ap.add_argument('-w', '--weights', 
                help = 'path to pre-trained weights', default="../yolov3-marker.weights")
ap.add_argument('-cl', '--classes', 
                help = 'path to objects.names',default="../object.names")
args = ap.parse_args()
...
window_title= "Landing Marker Detector"   
...
while cv2.waitKey(1) < 0 or False:
    ...
    label2 = 'Center Coordinates: X=%d, Y=%d' % (((x+w)-x)/2, ((y+h)-y)/2)
    cv2.putText(image, label, (0, 15), cv2.FONT_HERSHEY_SIMPLEX, .6, (0, 0, 255))
    cv2.putText(image, label2, (0, 40), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 255))
